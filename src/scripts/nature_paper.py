...
def copy_imu_files():
    ...
    print("Copying ...")
    poo = Pool(12)
    poo.map(worker, copier)
    print("Done!")

if __name__ == '__main__':
    work_dir = "M:/Mocap/"
    out_dir = "I:/Meta/Mocap/"
    participants = [f for f in os.listdir(work_dir) if f.lower().startswith('p')]
    participants_size = []
    print("Scanning for data in: {0}".format(work_dir))
    for p in participants:
        trials = [f for f in os.listdir("{0}{1}".format(work_dir, p)) if os.path.isdir("{0}{1}/{2}".format(work_dir, p, f))]
        trials_size = []
        for t in trials:
            print("{0}: {1}".format(p, t))
            imus = [f for f in os.listdir("{0}{1}/{2}".format(work_dir, p, t)) if f.endswith(".trc")]
            imus_size = [os.path.getsize("{0}{1}/{2}/{3}".format(work_dir, p, t, f)) for f in imus]
            trials_size.append(np.sum(imus_size))
            pass
        participants_size.append(np.sum(trials_size))

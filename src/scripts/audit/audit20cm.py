...
if __name__ == '__main__':
    ...
    for p in session:
        ...
        for s in session[p]:
            ...
            files = [f for f in os.listdir(mypath) if f.endswith('exported.trc')]
            for f in files:
                infile = "{0}{1}/{2}/{3}".format(imu_folder, p, s, f)
                out_name=f.replace("_re-export.trc", ".trc")
                outfile = "{0}{1}/{2}".format(out_folder, p, out_name)
                print(infile)
                print(outfile)
                copyfile("{0}{1}/{2}/{3}".format(imu_folder, p, s, f), "{0}{1}/{2}".format(out_folder, idx, f))
            pass
        break
    pass